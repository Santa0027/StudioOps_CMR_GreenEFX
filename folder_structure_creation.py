...
# import json # Removed: json operations will be handled by the backend
...
# structure_file = "saved_structures.json" # Removed: structure saving will be handled by the backend
...
save_location_entry.insert(0, "Input Save Location")
...
def load_predefined_structures():
    ...
    # if os.path.exists(structure_file): # Removed: structure saving will be handled by the backend
    #     try:
    #         with open(structure_file, 'r', encoding='utf-8') as f:
    #             predefined_structures_data = json.load(f) # Removed: json operations will be handled by the backend
...
def load_selected_structure(event=None):
    ...
    if selected in predefined_structures_data:
        ...
        # lines = tree_to_lines(structure) # Removed: tree_to_lines is in backend/utils
        # folder_text.insert(tk.END, "\n".join(lines)) # Removed: tree_to_lines is in backend/utils
...
def add_subfolders():
    ...
    lines = folder_text.get("1.0", tk.END).splitlines()
    # tree = parse_structure_to_tree(lines) # Extracted to backend/utils/folder_structure_generator.py
    ...
    # for node in tree: # Tree operations moved to backend
    #     if add_to_tree(node):
    #         break
    # else:
    ...
    # new_lines = tree_to_lines(tree) # Extracted to backend/utils/folder_structure_generator.py
    folder_text.delete("1.0", tk.END)
    # folder_text.insert(tk.END, "\n".join(new_lines)) # Extracted to backend/utils/folder_structure_generator.py
    extract_folder_names()
...
def create_structure():
    ...
    # The create_folders logic is now handled by the backend
    ...
    # if create_folders(tree, project_dir): # This call will be made to the backend
...
def save_structure():
    ...
    # structure = parse_structure_to_tree(lines) # Extracted to backend/utils/folder_structure_generator.py

    # data = {} # Json operations moved to backend
    ...
    # data[name] = structure # Json operations moved to backend

    # try:
    #     with open(structure_file, "w") as f:
    #         json.dump(data, f, indent=4) # Json operations moved to backend
    messagebox.showinfo("Saved", f"Folder structure saved as '{name}'")
    # dropdown['values'] = load_predefined_structures() # Refresh dropdown - will load from backend
    # except Exception as e:
...
ttk.Button(button_frame, text="Move Folder Down", width=20).pack(pady=8)
