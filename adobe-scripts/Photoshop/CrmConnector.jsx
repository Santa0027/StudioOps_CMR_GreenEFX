...
function createCrmConnectorPanel() {
    ...
    var buttonsGroup = taskListGroup.add("group");
    var refreshBtn = buttonsGroup.add("button", undefined, "Refresh");
    var openFolderBtn = buttonsGroup.add("button", undefined, "Open Project Folder");
    openFolderBtn.enabled = false;

    taskList.onChange = function() {
        openFolderBtn.enabled = (taskList.selection !== null);
    }
    ...
    openFolderBtn.onClick = function() {
        ...
        var selectedTask = tasksData[taskList.selection.index];
        var projectPath = SERVER_BASE_PATH + selectedTask.project_name.replace(/ /g, "_");
        var folder = Folder(projectPath);
        if (folder.exists) {
            folder.execute();
        } else {
            alert("Project folder not found at:\n" + projectPath);
        }
    };
    ...
    function fetchAndDisplayTasks() {
        ...
        if (response.status === 200) {
            ...
            tasksData = tasks; // Store the full data
