...
function UserManagement() {
  ...
  const { user, backendUrl } = useAuth(); // Get user and backendUrl from AuthContext

  const fetchUsers = async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await fetch(`${backendUrl}/api/users/users/`, {
        headers: {
  ...
  useEffect(() => {
    if (user && user.tokens && backendUrl) {
      fetchUsers();
    }
  }, [user, backendUrl]); // Re-fetch if user or backendUrl changes
  ...
  const handleDeleteUser = async (userId) => {
    ...
    try {
      const response = await fetch(`${backendUrl}/api/users/users/${userId}/`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${user.tokens.access}`,
